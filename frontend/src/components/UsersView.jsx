            ...
            {loading ? <div>{t('loading')}</div> : error ? <div className="text-danger">{error}</div> : (
                <>
                    <table className="table table-bordered table-hover">
                        ...
                        <tbody>
                            {users.length === 0 ? <tr><td colSpan="9">{t('noUsers')}</td></tr> : users.map(u => (
                                <tr key={u.id}>
                                    <td>
                                        {u.profilePhoto ? (
                                            <img src={`data:image/webp;base64,${u.profilePhoto}`} alt="avatar" className="rounded-circle" width={40} height={40} />
                                        ) : (
